...
// Get all teachers
$teachers = $conn->query("
    SELECT u.id, u.full_name, u.login_id, u.email 
    FROM users u 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($teachers as $teacher) {
        ...
        $status = $_POST['status'][$teacher_id] ?? 'absent';
        ...
        $existing = $conn->query("
            ...
            WHERE teacher_id = $teacher_id AND date = '$attendance_date'
        ")->fetch_assoc();
        
        if ($existing) {
            // Update existing record
            $stmt = $conn->prepare("UPDATE teacher_attendance SET status = ?, remarks = ? WHERE id = ?");
            $stmt->bind_param("ssi", $status, $remarks, $existing['id']);
        } else {
            // Insert new record
            $stmt = $conn->prepare("INSERT INTO teacher_attendance (teacher_id, date, status, remarks) VALUES (?, ?, ?, ?)");
    ...
    if ($success_count > 0) {
        $_SESSION['success'] = "Teacher attendance updated successfully for $success_count teachers!";
    }
    if ($error_count > 0) {
        $_SESSION['error'] = "Failed to update attendance for $error_count teachers.";
    }
    
    header("Location: teachers.php?date=$attendance_date");
    exit;
}
...
if ($teachers) {
    ...
    $result = $conn->query("
        ...
        WHERE teacher_id IN ($teacher_ids_str) AND date = '$selected_date'
    ");
...
$monthly_summary = $conn->query("
    ...
    WHERE u.role = 'teacher'
    GROUP BY u.id, u.full_name, u.login_id
    ORDER BY u.full_name
")->fetch_all(MYSQLI_ASSOC);
?>
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-blue-200">Teacher Attendance Management</p>
                    </div>
                </div>
                <div class="flex items-center space-x-4">
    ...
    <div class="container mx-auto px-6 py-8 flex">
        ...
        <aside class="w-64 flex-shrink-0">
            <nav class="bg-white rounded-lg shadow-md p-4 sticky top-4">
                <ul class="space-y-2">
                    ...
                    <li>
                        <a href="teachers.php" class="flex items-center space-x-3 p-3 rounded-lg bg-blue-50 text-blue-700">
        ...
        <main class="flex-1 ml-8">
            ...
            <?php if (!empty($teachers)): ?>
                <!-- Attendance Form -->
                <div class="bg-white rounded-lg shadow-md p-6 mb-6">
                    <h2 class="text-xl font-bold mb-4 text-gray-800">Mark Teacher Attendance</h2>
                                    ...
                                    <?php foreach ($teachers as $teacher): ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                <?= htmlspecialchars($teacher['login_id'] ?? '') ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <select name="status[<?= $teacher['id'] ?>]" class="px-3 py-2 border border-gray-300 rounded-md">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <input type="text" name="remarks[<?= $teacher['id'] ?>]" value="<?= htmlspecialchars($remarks) ?>" 
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php foreach ($monthly_summary as $teacher): ?>
                                    <?php
                                    $total_days = $teacher['present_days'] + $teacher['absent_days'] + $teacher['leave_days'];
                                    $attendance_percentage = $total_days > 0 ? round((($teacher['present_days'] + $teacher['leave_days']) / $total_days) * 100, 1) : 0;
                                    ?>
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900"><?= htmlspecialchars($teacher['full_name'] ?? '') ?></div>
            ...
            <?php else: ?>
                <div class="bg-white rounded-lg shadow-md p-6">
                    <p class="text-gray-500 text-center py-8">No teachers found in the system.</p>
                </div>
            <?php endif; ?>
        </main>
    </div>
